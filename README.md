...
Aircraft registration is public, so the reference lookup also fills the table on demand: when a registration is not yet known, `aircraftLookup.ts` asks a free public registry (adsbdb), normalises what it returns, and caches it as an aircraft record. The result is a prefill the pilot can correct, not an authority, and the cached record then becomes part of the maintained set so the flight entry validates against it. The normaliser is unit tested; the network call itself only runs in a deployment with outbound access.
...
The export uses a standard page size, A4 or US Letter, chosen by the holder in their account settings and stored on their profile. The grid is drawn at its natural width and then scaled to fit the chosen page, so the layout is identical on either size; only the sheet around it changes. The grid pages are landscape and the appendix that carries the sign-offs and the change log is the same paper in portrait. Every time printed on the grid carries a `Z` suffix to mark it as UTC.
...
- `POST /api/entries/{id}/sign` countersigns and locks an entry (account signer, with an MFA step-up). The signer must hold a permitting role, have the second factor enabled, and present a current code. `POST /api/entries/batch/sign` with `{ entryIds: [...] }` signs several at once after a single step-up (FOCA 2.4.2).
- `POST /api/entries/{id}/request-signoff` (holder) creates a one-time link for an external signer and emails it if SMTP is configured. `GET` and `POST /api/signoff/{token}` are the public, token-authenticated pages where that signer reviews and signs without an account.
...
- `GET /api/reference/{airports|aircraft}?q=` searches the reference databases; `GET /api/reference/aircraft?registration=HB-SFU` looks one aircraft up by registration, falling back to a public registry and caching the result; `POST /api/reference/{airports|aircraft|fstd}` adds a record (administrator only). Places and aircraft on an entry are validated against these.
...
Signing works two ways. A signer with an account countersigns with an MFA step-up. A signer without an account (an instructor who already uses another logbook and will not register) can be sent a one-time link: the holder requests a sign-off with the signer's name, email and capacity, the system creates a single-use, expiring token (only its hash is stored), emails the link over SMTP if configured and always returns it to share, and the signer opens a public page, confirms their name, optionally adds a licence number, draws their signature, and signs. The entry locks either way.

The handwritten signature image (2.4.3) is captured in the web app: an instructor or examiner draws their signature on a canvas when they countersign, and it is stored on the (append-only) signature record. An external signer has no account, so that signature row carries a null user id and the signer's name, email and licence as a snapshot; a throwaway key pair signs the content hash for tamper-evidence. What remains is not backend code. Route 1.3 finishes with a FOCA process: a declaration of conformity, testing against a FOCA test account, the dLIS data format which FOCA releases only after acceptance, an acceptance letter, and fees; the acceptance decision is FOCA's. A small follow-up is embedding that signature image into the printed PDF export (it is stored and shown in the app today).
